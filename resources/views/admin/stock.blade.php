                  ...
                  <table class="table datatables" id="dataTable-1">
                    <thead>
                     ...
                     <tbody>
                        ...
                        $sr=0;
                        @endphp
                        @foreach ($stock->unique('stock_number') as $data)
                        @php
                            $sr++;
                             $branch_to = branch::where(['id'=>$data->to])->first();
                             $branch_to_name = $branch_to->name;
                             $branch_from = branch::where(['id'=>$data->from])->first();
                      ...
                      <tr>
                        <td>{{$sr}}</td>
                        <td>{{$data->date}}</td>
                        <td>{{$branch_from_name}}</td>
                            <td>{{$branch_to_name}}</td>
                        ...
                        <td> <a class="btn btn-success btn-sm text-white" href="{{url('stock_bill',[$data->stock_number])}}" id="bill_view" data-bvid="{{$data->purchase_number}}">View</a></td>
                        <td>
                          <a class="btn btn-primary btn-sm" href="#" id="stock_edit" data-seid="{{$data->stock_number}}">Edit</a></td>
                        <td>
                          <a class="btn btn-danger btn-sm" href="#" id="stock_remove" data-srid="{{$data->stock_number}}">Remove</a></td>
...
html_data+='</div></div>'
 ...
 html_data +='</select>';
  ...
  html_data+='<div class="col-md-12"><div class="form-group">';
...
let id = $(this).attr("id");
  $("#attr_id"+id).remove();
    ...
    $(document).ready(function(e){
      $(document).on("click","#stock_remove",function(e){
